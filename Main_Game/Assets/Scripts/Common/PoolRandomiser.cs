...
public abstract class PoolRandomiser< T > : ScriptableObject
{
    ...
    private static void OnLoad()
    {
        ...
        PoolRandomiser< T >[] allPools = Resources.FindObjectsOfTypeAll< PoolRandomiser< T > >();
        Array.ForEach( allPools, pool => { m_AllPools.Add( pool.name, pool ); } );
    }

    public static bool TryGetPool( string a_Name, out PoolRandomiser< T > o_Pool )
    {
    ...
    private static Dictionary< string, PoolRandomiser< T > > m_AllPools;
}
